...
| 테스트 | 511개 통과 (도메인·서비스·MVC·HTTP 통합·반응형 계약) |
...
OMS 자동 테스트 511개가 통과했고, 정상 재입고·부분 승인·폐기·취소·장애 복구·보안 계약을 포함한
...
체크아웃은 주문을 `결제 대기`로 만든 뒤 별도 결제 페이지에서 모의 승인을 수행하고, 승인된 주문만 WMS에 할당합니다. 결제 실패는
...
요청할 수 있고 근거 없이 완료 처리할 수 없습니다. 성공한 환불은 게이트웨이 거래번호를 저장해 관리자
화면에서 요청 키와 함께 추적합니다.
