...
class MainControllerMvcTest {
    ...
    private Page<ProductCardDto> pageOf(ProductCardDto card) {
        return new PageImpl<>(List.of(card), PageRequest.of(0, 10, Sort.by("id")), 1);
    }
    ...
    void 가용재고가_없는_상품은_입고대기로_표시되고_주문은_가능하다() throws Exception {
        when(productService.findCardPage(eq(""), any(Pageable.class))).thenReturn(pageOf(card(0)));
    ...
    void 가용재고가_충분하면_입고대기와_남은수량을_표시하지_않는다() throws Exception {
        when(productService.findCardPage(eq(""), any(Pageable.class))).thenReturn(pageOf(card(50)));
        when(orderService.findOrders(1L)).thenReturn(List.of());
    ...
    void main은_keyword와_pageable로_상품페이지를_조회한다() throws Exception {
        Page<ProductCardDto> page = pageOf(card(50), PageRequest.of(1, 4, Sort.by("id")), 9);
        when(productService.findCardPage(eq("상품"), any(Pageable.class))).thenReturn(page);
        when(orderService.findOrders(1L)).thenReturn(List.of());
        ...
        verify(productService).findCardPage(eq("상품"), pageableCaptor.capture());
    ...
    void main은_keyword가_없으면_기본_페이지로_조회한다() throws Exception {
        when(productService.findCardPage(eq(""), any(Pageable.class)))
        ...
        ArgumentCaptor<Pageable> pageableCaptor = ArgumentCaptor.forClass(Pageable.class);
        verify(productService).findCardPage(eq(""), pageableCaptor.capture());
    ...
    void 페이지_윈도우는_현재_페이지를_중심으로_최대_5개를_노출한다() throws Exception {
        ...
        Page<ProductCardDto> page = pageOf(card(50), PageRequest.of(4, 10, Sort.by("id")), 120);
        when(productService.findCardPage(eq(""), any(Pageable.class))).thenReturn(page);
        when(orderService.findOrders(1L)).thenReturn(List.of());
    ...
    void 마지막_페이지에서는_윈도우가_끝에_붙는다() throws Exception {
        ...
        Page<ProductCardDto> page = pageOf(card(50), PageRequest.of(11, 10, Sort.by("id")), 120);
        when(productService.findCardPage(eq(""), any(Pageable.class))).thenReturn(page);
        when(orderService.findOrders(1L)).thenReturn(List.of());
    ...
    void 총_페이지가_5개_이하면_윈도우는_전체_페이지다() throws Exception {
        ...
        Page<ProductCardDto> page = pageOf(card(50), PageRequest.of(0, 10, Sort.by("id")), 20);
        when(productService.findCardPage(eq(""), any(Pageable.class))).thenReturn(page);
        when(orderService.findOrders(1L)).thenReturn(List.of());
