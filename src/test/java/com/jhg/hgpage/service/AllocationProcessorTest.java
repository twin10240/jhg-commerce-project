...
class AllocationProcessorTest {
    ...
    OrderAllocationService.AllocationCommand command =
            new OrderAllocationService.AllocationCommand(REQUEST_KEY, 3, Map.of(7L, 2));
    ...
    void 선점후_WMS를_호출하고_예약결과를_완료한다() {
        ...
        when(inventoryPort.reserveAll(REQUEST_KEY, 10L, command.quantities())).thenReturn(true);

        processor.process(10L);

        InOrder calls = inOrder(allocationService, inventoryPort);
        calls.verify(allocationService).claim(10L);
        calls.verify(inventoryPort).reserveAll(REQUEST_KEY, 10L, command.quantities());
    ...
    void 명시적_재고부족도_완료결과로_저장한다() {
        when(allocationService.claim(10L)).thenReturn(Optional.of(command));
        when(inventoryPort.reserveAll(REQUEST_KEY, 10L, command.quantities())).thenReturn(false);
    ...
    void 통신실패는_재시도하고_4xx는_즉시_검토로_보낸다() {
        ...
        when(inventoryPort.reserveAll(REQUEST_KEY, 10L, command.quantities()))
                .thenThrow(new ResourceAccessException("timeout"));
        when(inventoryPort.reserveAll(REQUEST_KEY, 20L, command.quantities()))
